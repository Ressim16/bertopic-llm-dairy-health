#!/usr/bin/env python3
"""
Step 09: Train New BERTopic Model on Filtered Data (Re-run After Topic Filtering)

This script trains a NEW BERTopic model on filtered abstracts using the SAME
hyperparameters from Step 05. This creates a fresh clustering on the filtered dataset.

Inputs:
    - abstracts_topic_filtered.csv: Filtered abstracts from Step 07
    - embeddings_rerun.npy: Re-generated embeddings from Step 08
    - grid_search_summary.json: Original hyperparameters from Step 04

Outputs:
    - bertopic_model_rerun/: Trained BERTopic model directory
    - topic_info_rerun.csv: Topic information
    - document_info_rerun.csv: Document-topic assignments
    - topic_assignments_frozen_rerun.csv: Frozen assignments
    - clustering_summary_rerun.json: Training summary
    - step_09_train_rerun_model.log: Execution log
...
def load_hyperparameters(grid_search_summary: Path, logger: logging.Logger) -> dict:
    """
    Load hyperparameters from grid search results or use provided values.
    ...
    """
    if grid_search_summary is None:
        logger.warning("No grid search summary provided, will use defaults or manual parameters")
        return {}
    ...
    logger.info("Hyperparameters from grid search:")
    for key in required_keys:
        logger.info(f"  {key}: {params[key]}")

    if 'n_topics' in params:
        logger.info(f"  Expected topics (from original): ~{params['n_topics']}")
    if 'coverage' in params:
        logger.info(f"  Expected coverage (from original): ~{params['coverage']:.4f}")
...
def load_data(input_file: Path, text_column: str, logger: logging.Logger) -> tuple:
    ...
    logger.info(f"Loaded {len(df):,} filtered rows")
...
def train_bertopic(
    ...
    logger.info("")
    logger.info("Training BERTopic model on filtered dataset (RE-RUN)...")
...
def save_outputs(
    ...
    """
    # Save model
    model_path = output_dir / "bertopic_model_rerun"
    ...
    try:
        ...
        # Fall back to pickle format if safetensors fails
        logger.warning(f"Safetensors save failed ({e}), falling back to pickle format...")
        model_path_pkl = output_dir / "bertopic_model_rerun.pkl"
        ...
        model_path_minimal = output_dir / "bertopic_model_rerun_minimal"
        try:
    ...
    assignments_path = output_dir / "topic_assignments_frozen_rerun.csv"
    ...
    np.save(output_dir / "umap_embeddings_rerun.npy", topic_model.umap_model.embedding_)
...
def save_summary(
    ...
    summary = {
        "mode": "retrained_on_filtered_data",
        ...
        "hyperparameters_source": str(grid_search_path) if grid_search_path else "manual",
        "model_path": str(output_dir / "bertopic_model_rerun"),
        "outputs": {
            "topic_info": str(output_dir / "topic_info_rerun.csv"),
            "document_info": str(output_dir / "document_info_rerun.csv"),
            "frozen_assignments": str(output_dir / "topic_assignments_frozen_rerun.csv")
        }
    }

    summary_path = output_dir / "clustering_summary_rerun.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Step 09: Train new BERTopic model on filtered data with same hyperparameters",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Example usage:
    python 09_train_rerun_model.py \\
        --input-file /path/to/07_filter/abstracts_topic_filtered.csv \\
        --embeddings /path/to/08_re_embed/embeddings_rerun.npy \\
        --grid-search-summary /path/to/04_grid_search/grid_search_summary.json \\
        --output-dir /path/to/output/09_train_rerun \\
        --text-column Abstract \\
        --seed 42

Or with manual hyperparameters:
    python 09_train_rerun_model.py \\
        --input-file /path/to/07_filter/abstracts_topic_filtered.csv \\
        --embeddings /path/to/08_re_embed/embeddings_rerun.npy \\
        --output-dir /path/to/output/09_train_rerun \\
        ...
        --min-cluster-size 15 --min-samples 10
        """
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help='Input CSV from Step 07 (abstracts_topic_filtered.csv)'
    )
    parser.add_argument(
        '--embeddings',
        type=Path,
        required=True,
        help='Embeddings .npy file from Step 08 (embeddings_rerun.npy)'
    )
    parser.add_argument(
        '--output-dir',
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help='Grid search summary JSON from Step 04 (recommended)'
    )

    # Manual hyperparameters (optional, used if --grid-search-summary not provided)
    parser.add_argument('--min-df', type=float, help='Min document frequency for vectorizer')
    parser.add_argument('--max-df', type=float, help='Max document frequency for vectorizer')
    parser.add_argument('--n-neighbors', type=int, help='UMAP n_neighbors')
    parser.add_argument('--n-components', type=int, help='UMAP n_components')
    parser.add_argument('--min-cluster-size', type=int, help='HDBSCAN min_cluster_size')
    parser.add_argument('--min-samples', type=int, help='HDBSCAN min_samples')
    ...
    # Configure logging
    logger = configure_logging(args.output_dir / "step_09_train_rerun_model.log")
    ...
    # Log header
    logger.info("=" * 70)
    logger.info("STEP 09: TRAIN BERTOPIC MODEL ON FILTERED DATA (RE-RUN)")
    logger.info("=" * 70)
    ...
    try:
        ...
        if args.grid_search_summary:
            ...
            manual_params = ['min_df', 'max_df', 'n_neighbors', 'n_components',
                           'min_cluster_size', 'min_samples']
            missing = [p for p in manual_params if getattr(args, p.replace('-', '_')) is None]

            if missing:
                raise ValueError(
                    f"Either --grid-search-summary or all manual hyperparameters must be provided. "
                    f"Missing: {', '.join('--' + p for p in missing)}"
                )
            ...
            logger.info("Using manual hyperparameters:")
        ...
        logger.info("")
        logger.info("=" * 70)
        logger.info("TRAINING RESULTS (RE-RUN)")
        logger.info("=" * 70)
        ...
        logger.info(f"  - {args.output_dir}/bertopic_model_rerun/")
        logger.info(f"  - {args.output_dir}/topic_info_rerun.csv")
        logger.info(f"  - {args.output_dir}/document_info_rerun.csv")
        logger.info(f"  - {args.output_dir}/topic_assignments_frozen_rerun.csv")
        logger.info(f"  - {args.output_dir}/clustering_summary_rerun.json")
        logger.info(f"  - {args.output_dir}/step_09_train_rerun_model.log")
        logger.info("=" * 70)
