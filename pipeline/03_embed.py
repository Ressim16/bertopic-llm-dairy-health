#!/usr/bin/env python3
"""
Step 03: Generate Sentence Embeddings with Chunk-Mean Pooling

This script generates dense vector embeddings for abstracts using BioBERT.
For texts exceeding the token limit, it uses chunk-mean pooling:
1. Split text into overlapping chunks (512 tokens with 80% overlap)
2. Embed each chunk separately
3. Average chunk embeddings to get final embedding

Inputs:
    - abstracts_with_truncation.csv: Abstracts from Step 02

Outputs:
    - embeddings.npy: Dense embeddings (float16 or float32)
    - embedding_metadata.json: Metadata about embeddings
    - embedding_summary.txt: Human-readable summary
    - step_03_embed.log: Execution log
...
def save_outputs(
    ...
    with open(summary_path, 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("STEP 03: EMBEDDING GENERATION SUMMARY\n")
        f.write("=" * 70 + "\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Step 03: Generate embeddings with chunk-mean pooling",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Example usage:
    python 03_embed.py \\
        --input-file /path/to/02_truncate/abstracts_with_truncation.csv \\
        --output-dir /path/to/output/03_embeddings \\
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help='Input CSV from Step 02 (abstracts_with_truncation.csv)'
    )
    parser.add_argument(
        '--output-dir',
        type=Path,
    ...
    # Configure logging
    logger = configure_logging(args.output_dir / "step_03_embed.log")
    
    # Log header
    logger.info("=" * 70)
    logger.info("STEP 03: GENERATE EMBEDDINGS")
    logger.info("=" * 70)
    ...
    try:
        ...
        logger.info(f"  - {args.output_dir}/embeddings.npy")
        logger.info(f"  - {args.output_dir}/embedding_metadata.json")
        logger.info(f"  - {args.output_dir}/embedding_summary.txt")
        logger.info(f"  - {args.output_dir}/step_03_embed.log")
        logger.info("=" * 70)
